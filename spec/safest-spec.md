...
> **状态**：正式发布  
> **生效日期**：2026-06-30  
> **对应 Coq 文件**：`safest.v`（形式化镜像）  
> **对应实现文件**：`lexer.v` `parser.v` `typechecker.v` `desugar.v`  
...
| 函数调用 | ✅ 包含 | 无副作用单表达式 FUNCTION 在编译期按位置传参并展开 |
...
**实现限制（v1.2）**：当前 VeriSTC 对 FUNCTION 调用采用编译期展开。
可展开的 FUNCTION 必须只有一个赋值语句，且赋值目标为函数名，例如
`Add := a + b;`。需要局部变量、内部控制流或持久状态的逻辑应显式传入
`previous` 等状态参数，或由后续 FUNCTION_BLOCK 实例机制支持。
