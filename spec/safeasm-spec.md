...
| v1.0 | 2026-06-29 | 正式发布。补全完整指令集（覆盖全部 66 条指令）；完善各段详细编码格式；新增验证规则章节；完善运行时小步语义；新增编码/解码示例 | JIANG Wei |
| **v1.1** | **2026-06-30** | **新增影子质量内存段 (SEG_QUALITY)；IOMap 扩展 quality_offset 字段；新增质量传播指令模板；新增 LOAD8_U / STORE8 指令** | JIANG Wei |
...
4. **可验证格式** — CRC32 覆盖 6 字节文件头之后的 Section 数据，不含末尾 4 字节 CRC 字段
...
├──────────────────────────────────────────────────────┤
│  [Checksum]   CRC32         4 字节，覆盖 Section 数据       │
└──────────────────────────────────────────────────────┘
```
...
**全局变量同步**：编译器将 `VAR_GLOBAL` 放入 `SEG_GLOBAL`。热备快照
始终把该段的页面加入复制集，即使调用者尚未上报对应脏页。这样 RS 状态、
计数器、累加器、滤波器和状态机可在主备切换后继续运行。
...
```
安全注解:
  [0]      safety_level: uint8  安全等级 (0=SIL2, 1=SIL3；编译器默认 1)
...
WCET 汇总（紧跟在安全注解后，保持向后兼容）:
  [0-3]  wcet_func_count: uint32
  每个函数:
    [0-3]  func_idx:   uint32
    [4-7]  wcet_cycles: uint32  最差执行指令数
    [8-11] wcet_ns:     uint32  最差执行时间 (ns)
```

### 4.7 WCET Section (段类型 6, 可选副本)

该段与 Safety Section 末尾的 WCET 汇总内容相同，保留用于独立读取；
编译器默认同时生成两者。
...
SafeASM 运行和反汇编由同一个可执行程序提供：

```
svm [options] <file.sasm> [expected] [local_index]

  -d           输出 dump 后退出
  -n CYCLES    执行周期数，0 表示持续运行（默认 0）
  -p MS        周期毫秒数（默认 1000）
```

默认模式为运行模式；周期起始时间使用单调时钟调度。`-d` 输出模块
反汇编、安全元数据和 WCET，并验证末尾 CRC32。
...
| V3 | CRC32 覆盖 6 字节文件头后的 Section 数据，必须匹配；全零值仅为旧测试文件兼容 | 加载时 |
...
| V22 | `safe_stack_depth` 必须 > 0 且 ≤ `MAX_CALL_DEPTH`（= 256） |
...
| 最大调用深度 | 256 层 |
| 单函数参数 | 16 |
| 单函数局部变量槽（参数加普通局部变量） | 1024 |
| 线性内存 | 1 MiB，最多 262144 个 i32 值 |
| 值栈深度 | 1024 |

`safe_stack_depth` 由编译器的调用图分析计算，表示所有函数路径中的最大
调用链深度，并写入 Safety Section。编译器还会拒绝递归调用，以及静态栈深
超过 256 的程序；因此 VM 的运行上限可以在装载前静态确认。
...
; --- Checksum ---
XX XX XX XX        ; CRC32 (覆盖 6 字节文件头后的 Section 数据)
```
