...
**定理 5（单步语义保持）**。对于编译成功的 SafeST 程序，每个 SafeST 执行步都能在 SafeASM 中找到一串对应的指令。即：如果 SafeST 从状态 $\sigma$ 走一步到状态 $\sigma'$，且 SafeASM 的当前状态 $\tau$ 与 $\sigma$ 对应，那么 SafeASM 执行若干步后会到达一个与 $\sigma'$ 对应的状态 $\tau'$。

**证明思路**。对 SafeST 的每种语句类型分别构造对应的 SafeASM 指令序列，然后证明该序列的执行结果与 SafeST 的语义一致。以赋值语句为例：`x := e` 编译为 `compile_expr(e); LOCAL_SET(x)`。`compile_expr(e)` 将表达式 $e$ 的值压入值栈，`LOCAL_SET(x)` 将栈顶值写入变量 $x$ 的内存位置——这一过程与 SafeST 赋值语义中"计算表达式值 → 更新变量"的过程完全对应。对于条件分支语句，`IF e THEN s1 ELSE s2` 编译为 `compile_expr(e); BR_IF(else_target); compile(s1); JMP(end_target); else_target: compile(s2); end_target:`——`BR_IF` 根据 $e$ 的求值结果决定跳转路径，与 SafeST 的条件分支语义一致。

**定理 6（整体语义保持）**。定理 5 的传递闭包版本：整个 SafeST 程序的执行过程（从初始状态到终止状态），在 SafeASM 中都能找到一条完整的对应执行路径。

**证明思路**。从初始状态开始，反复应用定理 5：每一步 ST 执行都能在 ASM 中找到对应段，将这些段首尾相连就得到完整的 ASM 执行路径。用数学归纳法可以严格证明这一结论。
## 7 类型安全性的 Coq 证明框架
