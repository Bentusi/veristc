...
P1-P4 框架不仅适用于数据类型和 POU 级别的剪裁决策，同样可以系统性地应用于 IEC 61131-3 表达式层面。表达式剪裁的目标是建立一个最小完备的表达式集：一方面保留的表达式必须覆盖安全级仪控系统所需的全部计算范畴——包括算术运算（PID 调节）、比较运算（联锁条件判断）、逻辑运算（保护系统表决）、数组索引（批量通道选择）和函数调用（模块化复用）；另一方面每个保留构造必须满足 P2（编译期可判定）、P3（Coq 可建模）和 P4（WCET 可计算），且存在至少一个不可替代的安全级用例（P1）。

被排除的表达式构造则需要满足更严格的条件：要么违反 P1-P4 中的至少两条，证明其从形式化验证的角度不可接受；要么虽然满足全部判据但存在语义等价的替代方案，因而保留它们只会增加编译器复杂度而不增加表达能力。以下按保留和排除两类分别展开。

#### 保留的表达式构造

保留表各列的含义如下。**范畴**列对表达式按功能进行分类，便于读者建立整体认知。**保留构造**列给出具体的语法形式，包括字面量、变量引用、数组索引、一元运算、二元运算、比较运算、逻辑运算和函数调用共八类。**P1 用例**列为每个保留构造提供一个典型的安全关键应用场景，用以验证该构造满足 P1 原则。**P2 可判定**、**P3 可建模**和 **P4 WCET** 三列分别标记该构造是否满足对应的剪裁判据，括号内给出具体的判定理由。

| 范畴 | 保留构造 | P1 用例 | P2 可判定 | P3 可建模 | P4 WCET |
|------|---------|---------|:---------:|:---------:|:-------:|
| 字面量 | 整数/实数/布尔/时间 | 常量定义 | ✓(编译期已知) | ✓ | ✓ |
| 变量引用 | 符号变量名 | 传感器值引用 | ✓ | ✓ | ✓ |
| 数组索引 | `a[e]` | 批量通道选择 | ✓(边界静态检查) | ✓(Coq 数组类型) | ✓(固定偏移) |
| 一元运算 | `-`, `NOT`, `ABS` | 取反/绝对值 | ✓ | ✓ | ✓ |
| 二元运算 | `+`, `-`, `*`, `/`, `MOD` | PID 运算 | ✓(类型提升可判定) | ✓ | ✓ |
| 比较运算 | `=`, `<>`, `<`, `>`, `<=`, `>=` | 联锁条件 | ✓ | ✓ | ✓ |
| 逻辑运算 | `AND`, `OR`, `XOR` | 保护系统表决 | ✓(短路不影响类型) | ✓ | ✓ |
| 函数调用 | `f(e₁,...,eₙ)` | 模块化复用 | ✓(编译期签名匹配) | ✓(Coq 函数模型) | ✓(调用图静态) |

从上表可以看出，八个保留构造在 P2、P3、P4 三列上全部为 ✓，这说明它们在形式化验证框架下都是良好行为（well-behaved）的。特别值得注意的是二元运算的类型提升和逻辑运算的短路求值：前者的类型提升规则（promote 函数，见第 3.3 节）在编译期可唯一确定结果类型；后者的短路求值虽然影响运行时控制流，但在类型层面不影响静态判定，AND 和 OR 的返回类型始终为 BOOL*。

这八类表达式构造与第 3.1.2 节定义的抽象语法严格对应，构成了 SafeST 表达式系统的全部语法范畴。它们之间的层次关系由优先级和结合性规则定义（见第 5 章具体语法），形成了从字面量到复杂嵌套表达式的完整构造层级。

#### 排除的表达式构造

排除表各列的含义如下。**构造**列给出被排除的 IEC 61131-3 表达式特性名称。**违反判据**列标记该构造所违反的 P1-P4 判据，若某个判据标记为"不足"，表示该判据并非完全违反而是不满足必要条件。**违反理由**列用简练的语言给出每条判据被违反的具体原因，为保守起见采用最严格的解读。**示例**列给出一段典型的 IEC 61131-3 代码片段，展示该构造在实际编程中的使用方式。**替代方案**列给出 SafeST 中推荐的等价写法，若完全禁止则标注"无"。

| 构造 | 违反判据 | 违反理由 | 示例 | 替代方案 |
|------|---------|---------|------|---------|
| ADR/REF/^ 取地址与间接引用 | P1,P2,P3,P4 | 安全级控制逻辑通过符号变量名直接引用 I/O 信号，不存在必须通过地址间接访问传感器的用例；指向分析在最一般情况下不可判定（Landi 1992），等价于停机问题；指针的形式化模型需要偏序堆配合分离逻辑（Reynolds 2002），其证明复杂度远超安全级程序的实际需要；间接访存的执行时间因地址动态绑定无法在编译期确定 | `p := ADR(x); y := p^` | 直接变量引用 `y := x` |
| %IW/%QW 直接硬件地址 | P1,P2 | 硬件地址与逻辑变量名的绑定属于系统集成层的配置问题，不应出现在控制逻辑表达式中；地址的解析依赖运行时的 I/O 配置表，编译期无法确定地址值 | `x := %IW3.1` | IOMap Section + 符号变量名 |
| CONCAT/LEFT/MID 字符串操作 | P1,P2,P4 | 安全级逻辑无需处理字符串，人机交互字符串由上层的 HMI 系统独立管理；字符串操作的结果长度依赖运行时输入，无法在编译期确定；字符串操作的执行时间与操作数的实际长度相关，无确定性上界 | `s := CONCAT(a, b)` | 排除，同 STRING 类型一并排除 |
| SHL/SHR/ROL/ROR 移位 | P1(不足) | 位串移位在安全级系统中确有应用场景（如状态字解析），但这些操作可以通过按位 AND、OR 和 NOT 的组合来实现，因此不存在不可替代的用例；移位位数若为运行时变量则同时违反 P2，但固定常量移位在编译期可完全确定 | `x := SHL(a, b)` | 组合 AND/OR/NOT（当前版本）；以内置函数形式引入（后续扩展） |
| `__ISVALID`/TYPEOF 运行时查询 | P1,P2,P3 | 运行时类型和状态查询的本质是运行时诊断，已被 Q*类型的静态质量码体系完全覆盖，无需单独的语言构造；运行时状态显然不能在编译期静态确定；反射语义在 Coq 的依赖类型理论中无法直接建模 | `IF __ISVALID(x)` | `Q_GOOD(x)` 或 `Q_STATUS(x)` |

从排除表可以归纳出三条观察。第一，最严重的一类构造（如 ADR/REF/^）同时违反全部四条原则，这类构造在安全级语言中没有任何存在的合理性依据。第二，部分构造（如 %IW/%QW 直接地址）虽然只违反两条判据，但被违反的恰好是安全关键系统的核心要求——P1 确保语言的领域相关性，P2 确保编译期的可决策性，两者缺一不可。第三，SHL/SHR 等移位操作属于"当前排除但保留扩展可能"的特殊情况，其排除理由不是 P1-P4 的硬性违反，而是 P1 的必要条件不满足——存在替代实现方案。这类构造将在编译器达到一定成熟度后，以内置函数而非运算符重载的形式逐步引入。

#### 形式化结论

综合保留和排除两张表，SafeST 的表达式剪裁满足以下形式化性质：保留的八类构造在 P2-P4 上全部标记为 ✓，且每一类都有至少一个明确的安全关键用例（P1）；被排除的五类构造中，四类违反 P1-P4 中的两条以上，一类（移位操作）虽未硬性违反但存在等价替代方案。因此，SafeST 的表达式系统在 P1-P4 框架下是完备的——不存在既满足全部判据又无法被八个保留构造表达的表达式需求。

### 3.5 语句的形式化剪裁

语句剪裁遵循与表达式剪裁一致的基本原则，但面临两个额外的形式化挑战。第一个挑战是控制流的结构化完备性：保留的语句集在 Böhm-Jacopini 定理（1966）的意义下必须足以表达任意控制逻辑，不能因为追求形式化可验证性而牺牲语言的表达能力。第二个挑战是循环终止性的编译期判定：安全关键系统中不允许存在非终止计算，因此 WHILE 和 REPEAT 循环必须带有编译期可验证的终止性证明机制，而非像通用语言那样依赖运行时监控或简单地假设循环终止。

SafeST 的语句剪裁策略是在接受这两个挑战的前提下，对 IEC 61131-3 的全部语句构造逐条进行 P1-P4 审查，最终得到九条保留语句和六条排除语句。

#### 保留的语句构造

保留表各列的含义如下。**构造**列给出语句的语法形式和简要说明。**用途**列描述该语句在安全级控制逻辑中的典型角色。**安全约束**列列出使用该语句时必须满足的编译期检查条件，违反这些条件将导致编译错误而非运行时异常，这正是 SafeST 将安全属性前移至编译期的体现。**P2/P4 保证**列简要说明该语句为何同时满足 P2 和 P4，括号内给出数学上的保证依据。

| 构造 | 用途 | 安全约束 | P2/P4 保证 |
|------|------|---------|:----------:|
| 赋值 `x := e` | 变量更新，控制逻辑中最基本的计算单元 | 赋值双方类型一致，Q*类型的质量码按规则传播 | ✓（单步操作，执行时间固定） |
| IF/ELSIF/ELSE | 条件分支，实现选择性的控制流 | 条件表达式类型必须为 BOOL* | ✓（分支路径数为常数 2，WCET 取最长路径） |
| CASE | 多路分支，实现整数状态机的状态转移 | 选择表达式类型必须为整数类型（SINT/INT/DINT/LINT） | ✓（分支数在编译期固定，WCET 为所有分支时间的最大值） |
| FOR | 有界循环，遍历固定次数的迭代操作 | 循环上下界为编译期常量或静态表达式；循环体内禁止对循环变量赋值 | ✓（迭代次数 = max(0, 上界 − 下界 + 1)） |
| WHILE | 条件循环，在条件满足时反复执行 | 条件表达式为 BOOL*；必须带有 Loop Variant 注解，该注解为一个从程序状态到良基集的严格递减函数 | ✓（良基递减函数保证循环在有限步内终止） |
| REPEAT | 后测试循环，至少执行一次 | 条件表达式为 BOOL*；必须带有 Loop Variant 注解 | ✓（至少执行一次后由 Variant 保证终止） |
| FB 调用 | 模块化封装，调用预先声明好的功能块实例 | FB 实例在 VAR 块中静态声明，传参方式为名称绑定 | ✓（调用图在编译期完全已知，无间接调用） |
| RETURN | 提前返回，从 FUNCTION 中提前退出 | 仅允许在 FUNCTION 体内使用 | ✓（不影响控制流的结构化性质） |
| EXIT | 循环中断，跳出最内层循环 | 仅允许在 FOR/WHILE/REPEAT 循环体内使用 | ✓（仅跳出最内层一层循环） |

从保留表可以归纳出 SafeST 语句系统的两个设计特征。其一，三种循环语句覆盖了所有可能的循环模式：FOR 适用于迭代次数已知的批量计算，WHILE 适用于前置条件判断的循环，REPEAT 适用于至少执行一次的后置条件循环。三种循环各有其不可替代的用例，同时都通过编译期机制（FOR 的上下界常量、WHILE/REPEAT 的 Loop Variant）保证了终止性。其二，FB 调用是 IEC 61131-3 领域特有的语句构造，它在通用语言中没有直接对应物。SafeST 保留了 FB 调用但施加了严格的静态实例化约束——所有 FB 实例在编译期声明，避免了运行时的动态创建。

#### 排除的语句构造

排除表各列的含义与表达式排除表相同。**构造**列给出被排除的 IEC 61131-3 语句特性名称。**违反判据**列标记所违反的 P1-P4 判据。**违反理由**列逐条说明违反的原因。**示例**列展示典型代码片段。**替代方案**列给出 SafeST 中的等价写法。

| 构造 | 违反判据 | 违反理由 | 示例 | 替代方案 |
|------|---------|---------|------|---------|
| STEP/TRANSITION/ACTION 顺序功能图 | P1,P2,P3,P4 | SFC 的步进语义在多个转换条件同时满足时需要冲突解决策略，引入语义非确定性；并发步进需要交织模型，状态空间随步数指数增长；并行分支的执行路径数无法在编译期确定，WCET 分析不可行 | `STEP S1: N Action1; TRANSITION FROM S1 TO S2` | `CASE state OF ... END_CASE` 状态机 |
| JMP 无条件跳转与标签 | P1,P2,P3,P4 | IEC 61508-3 第 5.2 节明确禁止安全关键系统中使用非结构化控制流；间接跳转的目标地址在编译期不可判定，即使直接跳转也会破坏结构化归纳证明中良基推导树的结构；非结构化跳转导致控制流图包含不可达边和回边，WCET 路径分析需处理任意循环嵌套组合 | `JMP label` | IF/CASE/结构化循环 |
| `__NEW`/`__DELETE` 动态内存分配 | P1,P2,P3,P4 | 安全级系统中所有资源在初始化阶段静态分配，运行时不存在需要动态创建或销毁变量的场景；堆分配函数的返回值在编译期完全不可预测；带堆的运行时模型需要维护复杂的不变量，显著增加形式化证明的工作量 | `p := __NEW(INT)` | VAR 块静态声明 |
| TRY/CATCH/`__QUERY_EXCEPTION` 异常处理 | P1,P2,P3,P4 | 安全关键系统采用故障-安全原则——异常应驱动系统进入已知的安全态而非在异常被捕获后继续不可靠的执行；异常是否抛出依赖运行时数据，静态不可预测；异常的形式化语义需要异常状态和栈展开规则，证明需处理非局部控制流；异常处理时间随展开深度变化 | `TRY x := y / z; CATCH ...` | `IF z <> 0 THEN x := y / z ELSE x := 0` |
| `a, b := c, d` 多目标赋值 | P1(不足) | 多目标赋值从本质上说是语法糖，不存在单一赋值无法表达的不可替代安全用例 | `a, b := c, d` | `a := c; b := d` |
| `__codedescriptor` 内联非 ST 代码 | P1,P2,P3,P4 | 在安全关键系统中插入未经形式化验证的非 ST 代码，从信任链的角度来看等同于在数学证明中引入未经证明的前提，其后果是整个验证链条的断裂 | `__codedescriptor { /* C code */ }` | 完全禁止，无替代方案 |

从排除表可以得出两条结构性观察。第一，SFC、JMP、动态内存和异常处理这四类构造均同时违反全部四条原则，它们在 P1-P4 框架下没有任何保留的合理性。这些构造的排除不仅是工程直觉的判断，更是形式化推理的必然结论。第二，多目标赋值和内联代码代表了两种不同的排除类型：多目标赋值是典型的"语法糖排除"——它在形式化上无问题（其余三判据均满足），但不满足 P1 的必要条件，因此被排除以保持语言的最小性；内联代码则是"原则性排除"——它违反全部四条原则，没有任何妥协余地。

#### 形式化结论

保留的九条语句构造在 Böhm-Jacopini 意义下构成结构化控制流的最小完备集：赋值提供顺序执行，IF 和 CASE 提供条件分支，FOR、WHILE 和 REPEAT 提供三种不同模式的循环，FB 调用、RETURN 和 EXIT 作为领域特定和工程便利的扩充但均可约简为基本控制结构。被排除的六类构造均违反 P1-P4 中的至少一条（五类违反两条以上），且每类都存在结构化语义等价的替代方案或完全不具备在安全关键系统中使用的前提条件。
