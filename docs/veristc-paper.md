...
后续各节的安排如下。第 2 节形式化地定义安全级编程语言的语义要求,包括语义确定性和语义空洞的概念,并证明 WCET 可计算性的三个充分条件及其不可弥补性。第 3 节阐述影子类型体系,将质量提升为一等类型,定义两种质量传播模式(串行衰减 worst 和并行择优 best),给出 Q*类型的类型规则和影子内存的编译实现方案。第 4 节提出 P1-P4 四条剪裁判据及其形式化定义,证明剪裁完备性定理,并以指针为例展示如何在这四条原则的框架下对语言特性进行逐条判断。第 4.4 节和第 4.5 节进一步将剪裁框架系统地应用于表达式和语句层面,逐项分析被排除的 IEC 61131-3 构造并给出形式化剪裁理由。第 5 节给出 SafeASM 字节码的形式化设计,重点论述固定宽度编码的逻辑动机及其与 WCET 可计算性之间的理论关联。第 6 节阐述语义保持的证明策略，借鉴 CompCert 的三层架构设计，建立 SafeST 到 SafeASM 的语义对应关系，并给出核心定理的证明思路。第 7 节呈现类型安全性的 Coq 证明框架，包括三阶段证明架构的设计、Progress 和 Preservation 的证明策略，以及类型安全与语义保持之间的逻辑衔接。第 8 节将本文工作与 CompCert、PLCopen Safety、现有 WCET 分析方法以及质量语义的相关研究进行系统性比较。第 9 节讨论方法的普适性与当前工作的局限性,并展望后续研究方向。
...
如果语言 $\mathcal{L}$ 同时满足三个条件：$\mathcal{L}$ 是语义确定性的（定义 1），所有循环的终止性在编译期可判定，且所有指令的执行时间在编译期可界定，则 $\mathcal{L}$ 中任意程序 $P$ 的最差执行时间 $T_{WCET}(P)$ 是可计算的。
...
在安全级工业控制系统中，传感器信号的可靠性信息（即信号质量）与信号数值本身同等重要。一个来自故障传感器的"正常"读数比一个来自健康传感器的"异常"读数更加危险。然而，IEC 61131-3 标准以及现有的安全子集方案均未将信号质量作为语言层面的一等概念加以处理，质量检查被留给程序员的显式函数调用，质量传播则依赖手工程序维护。本节提出影子类型理论，将质量维度嵌入类型系统，使得质量传播成为编译期自动推导的过程而非运行时可遗漏的检查。
...
如果程序员只检查了 $a$ 的质量而忽略了 $b$ 和 $c$，当 $a$ 为 GOOD 但 $b$ 为 BAD 时，表决结果可能输出 TRUE，因为 $b \land c$ 分支基于不可信数据。这种错误在传统做法中无法被编译器捕获。

第二个问题是**不可组合性**：质量传播逻辑与控制逻辑交织在一起，难以独立验证。例如 PID 控制器的输出质量取决于三个输入的质量，但这种依赖关系分散在算法实现的各个条件分支中，无法被统一分析。将质量作为类型系统的一部分后，编译期自动跟踪每个变量的质量状态，程序员不再需要手动维护质量传播代码。
...
**Q*类型**。对于每个基础类型 $\tau$，定义其带质量位的版本 $\tau^*$，称为 Q*类型。SafeST 支持的 Q*类型如下：
...
Q*类型的值空间为 $\mathcal{V}_{\tau^*} = \mathcal{V}_\tau \times Q$，即每个变量同时包含一个数值和一字节质量码。质量码与值的宽度无关，固定占 1 字节。

**表达式类型规则**。以下规则说明质量如何作为一等类型在表达式间传播。$\Gamma \vdash e : \tau$ 表示在类型环境 $\Gamma$ 下，表达式 $e$ 的类型为 $\tau$。

字面量和变量：
- 字面量（如 `42`、`TRUE`）的质量固定为 GOOD
- 变量引用的质量从运行时读取变量的质量码
...
1. 普通类型可以**隐式**赋值给对应的 Q*类型，质量码自动设为 GOOD。例如 `q : QINT := 42` 合法，编译器将 42 提升为 QINT 并附加 GOOD 质量。这一规则方便了常量初始化和非质量感知代码的集成。

2. Q*类型**不能**隐式赋值给普通类型。将 Q*变量的数值提取到普通变量需要通过内置函数 `Q_VALUE(x)` 显式进行，该函数返回数值部分并丢弃质量码。例如 `raw : INT := Q_VALUE(q)`。这一设计防止质量信息在不知情时被丢弃——如果允许隐式转换，程序员可能意外使用一个 BAD 质量的信号值而不自知。

3. Q*类型之间可以相互赋值，前提是基础类型兼容（遵循 promote 规则）。赋值时质量码随之传递。例如 `a : QINT := b : QINT` 将 $b$ 的质量码完整复制到 $a$。
...
Q*类型在编译实现中采用**影子内存**方案。每个 Q*类型变量的数值存储在主数据区，质量码存储在独立的影子区：
...
1. **质量码与值宽度解耦**：质量码始终占 1 字节，与值的宽度（1–8 字节）无关，影子区总大小只与 Q*变量个数有关。
2. **WCET 确定性**：质量传播代码在编译期已插入，运行时不引入条件分支，执行路径固定。
3. **向后兼容**：非 Q*类型不占用影子区，已有存量代码的内存布局不变。
## 4 SafeST:安全子集的形式化剪裁
...
如果语言 $\mathcal{L}$ 的所有特性都满足 P1-P4,则 $\Phi(\mathcal{L}) = 0$,即 $\mathcal{L}$ 是语义无空洞的。

假设 $\mathcal{L}$ 的所有特性满足 P1-P4,但存在状态 $s \in \mathcal{S}$ 使得 $|\{ s' \mid s \xrightarrow{\mathcal{L}} s' \}| > 1$(非确定性)或 $\nexists s' \cdot s \xrightarrow{\mathcal{L}} s'$(未定义)。前者意味着存在某个特性 $f$ 的操作语义规则导致非确定性分支，但 P2 和 P4 联合排除了这种可能性，因为非确定性导致循环终止性不可判定（违反 P2）,且 WCET 不可计算(违反 P4)。后者意味着存在某个特性 $f$ 在某些输入下没有定义后继状态，但 P3 要求 $f$ 在 Coq 中有全函数（Coq 中所有函数是全的）的操作语义模型，因此不存在未定义状态。
...
上表所示的八类表达式构造与第 4.1.2 节定义的抽象语法严格对应，构成了 SafeST 表达式系统的全部语法范畴。下面对其中两类需要特别说明的构造展开讨论。
...
SafeASM 是 VeriSTC 编译器的目标语言，一种面向安全关键系统的定制字节码格式。与通用处理器指令集不同，SafeASM 的设计目标不是计算效率最大化，而是形式化可验证性：每条指令的语义可以在 Coq 中精确建模，最差执行时间可以在编译器输出字节码时直接计算而非事后分析。

### 5.1 固定宽度编码

SafeASM 指令集包含 66 条指令，每条指令采用**固定宽度编码**。不同类别的指令占用的字节数固定：
...
关键设计决策是使用固定宽度而非变长编码（如 WASM 的 LEB128）。动机是 WCET 可计算性：变长编码下指令的取指时间依赖于编码长度，而编码长度在存在数据依赖分支时无法静态确定。固定宽度编码保证每条指令的取指时间是常数，与指令本身无关。
...
其中 $N$ 是程序的总指令数，$t_{\text{fetch}}$ 是每条指令的取指时间（常数），$t_{\text{exec}}(i_j)$ 是指令 $i_j$ 的执行时间。由于所有循环有编译期可验证的上界且不存在递归调用，$N$ 可在编译期确定。因此 $T_{\text{total}}$ 在编译期可计算，无需事后进行 WCET 分析。这一结论的工程含义是：SafeASM 的 `.sasm` 文件中可以直接包含 WCET Section，虚拟机在加载时即可读取。
## 6 语义保持：编译器正确性的核心保证

编译器正确性的核心命题可以表述为：源语言中的每一条执行轨迹，在目标语言中都存在一条对应的执行轨迹，且两条轨迹的行为一致。对于 VeriSTC 而言，这意味着控制工程师在 SafeST 中编写的每一段控制逻辑，其编译后的 SafeASM 代码都忠实地保持了源程序的语义。

### 6.1 证明策略：参考 CompCert 的三层架构

VeriSTC 的语义保持证明借鉴了 CompCert 的架构设计。CompCert 将编译过程分解为多个中间语言层（Clight→C#minor→Cminor→…→Asm），每一层到下一层的变换独立证明语义保持，最终通过关系复合得到从源语言到目标语言的完整正确性定理。

VeriSTC 采用简化的两层架构：

```
SafeST 源程序  ──→  SafeASM 字节码
   │                    │
   ▼                    ▼
 ST 语义模型         ASM 语义模型
   │                    │
...
其中对应关系 $R$ 的定义基于以下观察：SafeASM 的每条指令可以归类为若干种模式（值栈操作、控制流操作、内存操作），每个 SafeST 语句编译后对应一个固定的 SafeASM 指令序列。因此 $R$ 只需要保证编译后的指令序列执行时，SafeASM 的内部状态与 SafeST 的语义状态一致。

### 6.2 对应关系的构造

对应关系 $R$ 建立了 SafeST 运行状态与 SafeASM 运行状态之间的联系。一个 SafeASM 状态"对应"一个 SafeST 状态，当且仅当满足以下条件：

1. **变量值一致**：SafeST 中每个变量的值等于 SafeASM 内存中对应位置的值
2. **质量码一致**：SafeST 中每个 Q*变量的质量码等于 SafeASM 影子内存中的对应字节
3. **调用栈一致**：SafeST 的调用栈深度等于 SafeASM 的帧栈深度

这三条条件的直观含义是：如果你在 SafeST 中单步执行到某处暂停，然后查看对应的 SafeASM 程序执行到对应位置时的内存状态，两者的数据应该完全一致。

### 6.3 核心定理

对于编译成功的 SafeST 程序，每个 SafeST 执行步都能在 SafeASM 中找到一串对应的指令。即：如果 SafeST 从状态 $\sigma$ 走一步到状态 $\sigma'$，且 SafeASM 的当前状态 $\tau$ 与 $\sigma$ 对应，那么 SafeASM 执行若干步后会到达一个与 $\sigma'$ 对应的状态 $\tau'$。

对 SafeST 的每种语句类型分别构造对应的 SafeASM 指令序列，然后证明该序列的执行结果与 SafeST 的语义一致。以赋值语句为例：`x := e` 编译为 `compile_expr(e); LOCAL_SET(x)`。`compile_expr(e)` 将表达式 $e$ 的值压入值栈，`LOCAL_SET(x)` 将栈顶值写入变量 $x$ 的内存位置——这一过程与 SafeST 赋值语义中"计算表达式值 → 更新变量"的过程完全对应。对于条件分支语句，`IF e THEN s1 ELSE s2` 编译为 `compile_expr(e); BR_IF(else_target); compile(s1); JMP(end_target); else_target: compile(s2); end_target:`——`BR_IF` 根据 $e$ 的求值结果决定跳转路径，与 SafeST 的条件分支语义一致。

定理 5 的传递闭包版本：整个 SafeST 程序的执行过程（从初始状态到终止状态），在 SafeASM 中都能找到一条完整的对应执行路径。

从初始状态开始，反复应用定理 5：每一步 ST 执行都能在 ASM 中找到对应段，将这些段首尾相连就得到完整的 ASM 执行路径。用数学归纳法可以严格证明这一结论。
## 7 类型安全性的 Coq 证明框架
...
Soundness 方向通过对可判定检查函数的递归结构做归纳，对每个分支构造出对应的 `has_type` 推导式。Completeness 方向对 `has_type` 的推导树做结构归纳，利用类型提升函数、一元运算符有效性函数等辅助函数的完备性引理，将归纳假设转化为可判定检查函数的成功返回值。
...
证明的核心是对语句结构做情形分析，对每个语句类别分别构造出一个可执行的一步。关键观察是：SafeST 的语义确定性使得每条转移规则的后继状态唯一确定，因此"存在后继状态"的证明等价于"转移规则的前提条件全部满足"的证明。
...
对表达式 $e$ 的结构做归纳。基础情形（字面量、变量引用）直接由状态定义可得。归纳情形中，每个子表达式由归纳假设保证有定义，且运算本身是全函数（二元整数运算在除零时返回 0 而非未定义，浮点运算对所有浮点输入有定义）。因此 $\llbracket e \rrbracket_\sigma$ 总是返回某个值。
...
Preservation 的论证分为两个层次。第一层是程序级保持，这是直接的：程序良类型性谓词是一个纯语法属性，仅依赖程序的抽象语法树和声明信息，不依赖任何运行时状态。由于一步执行不改变程序的抽象语法树，所有声明和类型标注维持不变，因此程序级的良类型性平凡地保持。
...
对执行规则的每种情形做分析。对于赋值规则，程序良类型性保证类型检查器对赋值语句返回成功，这意味着左端变量声明类型与右端表达式类型相容。由 $\text{Consistent}(\sigma)$ 和引理 1，表达式求值返回的类型与静态检查时的类型一致，因此新值的类型与声明类型相容，更新后的状态保持 $\text{Consistent}$。对于条件分支规则，不修改任何变量的值，因此 $\text{Consistent}$ 平凡保持。对于循环展开规则，`WHILE` 在小步语义中展开为条件判断加循环体加递归，不改变变量值集合，$\text{Consistent}$ 保持。对于循序复合规则，由归纳假设对子语句的保持性可得。
...
对于任意类型良好的 SafeST 程序 $P$ 和任意满足 $\text{Consistent}(\sigma_0)$ 的初始状态 $\sigma_0$：

$$\text{star\_step\_st}(P, \sigma_0, \sigma) \Rightarrow \text{terminal\_state}(\sigma) \lor \exists \sigma' \cdot \text{step\_st}(P, \sigma, \sigma')$$

对多步执行的步数 $n$ 做数学归纳。基例 $n=0$ 时，由 Progress 定理（若非终态则存在下一步）可得结论。归纳步骤中，设 $\sigma_0 \xrightarrow{ST}^n \sigma_n \xrightarrow{ST} \sigma_{n+1}$。由归纳假设，$\sigma_n$ 要么是终态（已得证），要么存在下一步。Preservation 定理保证 $\text{Consistent}(\sigma_n)$ 保持，因此 Progress 定理可再次应用，保证 $\sigma_n$ 若为非终态则存在下一步。
...
VeriSTC 通过直接将 SafeST 编译为 SafeASM 字节码,从根本上绕过了上述三个问题。由于不存在 C 语言中间表示,工具链中不需要引入任何未经验证的编译器组件;由于 SafeASM 的指令语义与 SafeST 的语义构造之间存在直接的对应关系(见第 6 节的语义保持关系),领域语义信息在编译过程中被保持而非丢弃;由于固定宽度编码保证了取指时间的静态可计算性(定理 4),WCET 信息可以作为编译器的直接输出而非事后分析的结果。此外,与 Lustre/SCADE 要求控制工程师学习新的同步数据流语言范式不同,IEC 61131-3 ST 是工业控制领域通用的编程语言,拥有庞大的存量代码库和成熟的工程师群体。VeriSTC 兼容已有 ST 控制代码的能力,使得安全升级可以在不改变工程团队工作流程的前提下渐进式地开展,从而降低了形式化验证技术在工业实践中推广的采用壁垒。
...
P1-P4 剪裁判据的设计独立于 IEC 61131-3 的具体语法特性，其核心是一组关于语言可验证性的充分条件，因此可以作为一种通用方法论应用于其他工业领域的形式化语言设计中。以 IEC 61499（功能块）为例，其组合语义天然适合操作语义建模，而事件语义的并发性则为 P1-P4 框架提供了一个自然的扩展方向——将 P4 的 WCET 可计算性要求从单线程扩展到并发场景，可以通过交织语义或偏序归约来实现，这恰好是形式化方法中已经充分研究的领域。对于 IEC 61850（变电站自动化），其抽象通信服务接口（ACSI）的语言模型中包含实时约束（如 GOOSE 报文的 4 ms 传输时限），这实际上是 P4 的一种自然推广——将 WCET 从单机范围延伸至网络范围，此时固定宽度编码的优势更为突出，因为网络传输中的确定性延迟同样受益于可预测的数据包大小。而对于 IEC 62541（OPC UA），其信息模型的类型系统可以沿用 P3 的形式化方法进行建模，相关建模技术为此提供了坚实的技术基础。上述推广方向表明：P1-P4 框架提供了一组灵活的判据，每个领域只需根据其特有的语义特征对判据的具体内涵进行适配即可。

### 9.2 形式化框架的自然延伸方向

当前证明框架在多个方向上可以自然地延伸和深化，这些延伸不是对框架的修正，而是将其推向更高形式化完备性的增强。

**从语义保持到全程序正确性的衔接。** 本文证明的语义保持关系构成了编译器正确性的核心。在此基础上，可以向全程序正确性（total correctness）的方向自然延伸——全程序正确性要求在语义保持的基础上，进一步在规范层面建立安全属性与程序语义之间的对应关系。VeriSTC 的证明框架可以自然地与程序验证方法衔接，形成"编译器正确性 + 程序逻辑"的双层验证架构：编译器保证编译后的代码忠实地反映了源程序的语义，程序逻辑则保证源程序本身满足安全规范。这一衔接层是本文框架最有价值的扩展方向之一。

**抽象函数的精细化与完备性刻画。** 模拟关系 $R$ 的定义依赖于从 SafeASM 状态空间到 SafeST 状态空间的抽象函数 $\alpha$，而抽象函数的选择本身就是一个可优化的设计空间。本文采用的四维抽象（变量值、质量码、调用栈深度、周期计数）是最小化的实用选择，在此基础上有多种精细化的可能：例如，可以增加内存访问模式维度以支持更细粒度的安全验证，也可以通过建立更精确的形式化对应关系来刻画抽象的完备性。不同的抽象层次适用于不同的验证目标，这种灵活性本身就是框架设计的一个优势。
...
**时序语义维度的嵌入。** SafeST 当前的语义模型覆盖了单周期数据流，而安全关键系统中的部分安全属性涉及跨周期的时序约束——例如"在停堆信号触发后的 50 ms 内完成控制棒插入"或"同一传感器在连续三个扫描周期内均输出超限值才能触发报警"。这类时序安全属性的形式化验证可以通过将实时逻辑或时间自动机的语义嵌入到 SafeST 的语义框架中来实现。本文的小步语义为这种嵌入提供了良好的基础：只需在状态空间中增加时间戳维度，并在转移规则中引入时间约束，即可将框架从纯数据流扩展为数据-时间混合语义。这一扩展方向在理论上是直接的，在工程上则对应 SafeASM 中 WCET Section 的丰富化。
...
**编码方案空间的探索。** 当前 SafeASM 采用固定宽度编码以保证 WCET 可计算性。在固定宽度与完全变长编码之间，存在一个丰富的中间地带值得探索。例如稀疏编码——大多数指令使用固定短码、少数指令使用长码——可能在保持 WCET 可计算性的同时改善代码密度。刻画这一编码方案空间的结构，确定 WCET 可计算性在该空间中的精确边界，是编译器形式化设计中一个既有理论趣味又有实践价值的问题。

**互模拟关系的建立。** 本文建立的语义保持关系（定理 5、6）保证了 SafeST 语义到 SafeASM 语义的忠实翻译。如果 SafeASM 的指令集设计中不存在"多余行为"（即每条 SafeASM 指令都可以映射回 SafeST 的某个语义构造），则语义保持可以强化为互模拟关系——不仅源语言的每条轨迹能在目标语言中找到对应，目标语言的每条轨迹也能在源语言中找到来源。这提供了更强形式的正确性保证。
## 参考文献
